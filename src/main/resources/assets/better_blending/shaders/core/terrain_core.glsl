...
vec3 surfaceTint(ivec2 cell, float height) {
    ...
    return pixel.x < 0 ? vec3(1.0) : texelFetch(SurfaceColors, pixel, 0).rgb;
}
...
float choiceNoise(vec3 world, vec3 key, int scale) {
    ...
    if (scale > 1 && faceNormal.y > 0.5)
        value = mix(noise((texel + 0.5) / 32.0 + seed), value, 0.35);
...
float neighborWeight(ivec2 cell, vec3 surface, float height, float scale, vec2 position) {
    ...
    return tent.x * tent.y * (1.0 - smoothstep(1.0, 3.0, abs(surface.y - height)));
}
...
vec4 blendTerrain(vec4 source, int ownMaterial, ivec2 cell, float height, int face,
    ...
    for (int z = -1; z <= 1; z++) for (int x = -1; x <= 1; x++) {
        ...
        if (int(data.x) != ownMaterial && connectedSurface(cell, neighbor, height, data.y)) {
            color = materialColor(int(data.x), face, uv, gradX, gradY, surfaceTint(neighbor, data.y));
